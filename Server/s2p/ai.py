...
with tf.device(strg0):
    dull_head = load_model('models/dull_head.net', compile=False)
    gate_head = load_model('models/gate_head.net', compile=False)
    line_head = load_model('models/line_head.net', compile=False)
    noise_tail = load_model('models/noise_tail.net', compile=False)
    clear_tail = load_model('models/clear_tail.net', compile=False)

with tf.device(strg1):
    base_head = load_model('models/base_head.net', compile=False)
    base_neck = load_model('models/base_neck.net', compile=False)
    base_reader = load_model('models/base_reader.net', compile=False)
